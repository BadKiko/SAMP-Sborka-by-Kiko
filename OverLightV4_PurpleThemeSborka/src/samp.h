...
struct stServerPresets
{
	...
	uint8_t byteUnk2[4];
	int		iClassesAvailable; // useful he he
...
struct stTrailerData // 54
{
	uint16_t	sTrailerID; //0 - 2 //currect
	float		fPosition[3]; // 2 - 14 //currect
	float		fQuaternion[4]; // 30 //currect
	float		fMoveSpeed[3]; // 42 //currect
	float		fTurnSpeed[3]; // 54 ok
};

/* no
struct stTrailerData2
{
	uint16_t	sTrailerID;
	float		fRoll[3];
	float		fDirection[3];
...
struct stLocalPlayer
{
	...
	BYTE     byteSpace3[262]; //check
	struct stSurfData  surfData; // offset + 2
...
struct stRemotePlayerData
{
	...
	uint8_t     byteTeamID;//1
	uint8_t     bytePlayerState; //1
	uint8_t     byteSeatID;//1
	uint32_t    ulUnk3;//4
	int      iPassengerDriveBy;//4
	void     *pUnk0;//4
	uint8_t     byteUnk1[60];//60
	float     fSomething[3];//12
	float     fVehicleRoll[4];//16
	uint32_t    ulUnk2[3];//16
	float     fOnFootPos[3]; // 16 - current: 127// make 206
	...
	short     sShowNameTag;
	int      iHasJetPack;
	uint8_t     byteSpecialAction; //currect
	/*uint8_t     ulUnk4[5];
	struct stAimData  aimData;
	struct stInCarData  inCarData;
	struct stOnFootData  onFootData;
...
struct stEnterVehicle
{
	short VehicleID;
	BYTE bytePassenger;
};

struct stExitVehicle
{
	short VehicleID;
};

struct stVehicleDestroyed
{
	int VehicleID;
};
...
// struct from my old project on 0.3z
struct stNewModSa
{
	//chat
	//stChatTranslater ChatTranslater;
	bool bImportantMessage;
	bool bPlayerMessageChat;
	...
	//infos
	int iVehiclesStreaming;
	int iPlayersStreaming;
	int iFriendsStreaming;
	//infos for actors or vehicles
	//HANDLE get_info_thread;
	//D3DXVECTOR3 ActorScreenPos[SAMP_MAX_PLAYERS];
	//int iActorSAMPID[SAMP_MAX_PLAYERS];
	//int iActors;

	//friends
	bool bIsFriend[SAMP_MAX_PLAYERS];
	int iFriendID[SAMP_MAX_PLAYERS];

	//admins
	bool bIsAdmin[SAMP_MAX_PLAYERS];
	int iAdminID[SAMP_MAX_PLAYERS];
...
extern struct stOL_Cheats						*OLCheats;

extern bool										bAntiBadVehicle;
extern bool										bAntiBadBullets;
extern bool										bAntiBadUnoccupied;
extern bool										bAntiBadAim;
extern bool										bAntiVehicleTroll;
