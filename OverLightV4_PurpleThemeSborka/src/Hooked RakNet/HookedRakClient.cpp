...
Packet2* HookedRakClientInterface::Receive(void)
{
	...
	if (ReceivedPacketID == ID_UNOCCUPIED_SYNC )
	{
		...
		if (bAntiBadUnoccupied)
		{
			...
			bsUnoccupiedSync.Read(IncomingUnoccupiedData.byteSeatID);

			bsUnoccupiedSync.Read(IncomingUnoccupiedData.fRoll[0]);
			bsUnoccupiedSync.Read(IncomingUnoccupiedData.fRoll[1]);
			bsUnoccupiedSync.Read(IncomingUnoccupiedData.fRoll[3]);

			bsUnoccupiedSync.Read(IncomingUnoccupiedData.fDirection[0]);
			bsUnoccupiedSync.Read(IncomingUnoccupiedData.fDirection[1]);
			bsUnoccupiedSync.Read(IncomingUnoccupiedData.fDirection[3]);
			...
			if (vect3_length(IncomingUnoccupiedData.fRoll) > 1000.0f)
				deSyncUno = true;

			if (vect3_length(IncomingUnoccupiedData.fDirection) > 1000.0f)
				deSyncUno = true;
