...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(strcmp(cmd, "/objlim1", true) == 0) {
	    ...
	    new lp=0;
 		GetPlayerPos(playerid, X, Y, Z);
 		while(lp != 999) {
	    	CreateObject(1656, X, Y, Z, 0.00, 0.00, 0.00, 30.0);
	    	Y += 0.25;
	    	lp++;
		}
	    return 1;
	...
	if(strcmp(cmd, "/testplain", true) == 0) {
	    ...
	    if(GetPlayerVehicleID(playerid)) {
		    SetVehiclePos(GetPlayerVehicleID(playerid), 38.20, 1797.33, 605.00);
	    }
		else {
		    SetPlayerPos(playerid, 38.20, 1797.33, 605.00);
	    }
	}
	...
	if(strcmp(cmd, "/clicktd",true) == 0) {
    	...
    	txtTestText1 = TextDrawCreate(320.0, 100.0, "Test Text 1"); // Text is txdfile:texture
	...
	if(strcmp(cmd, "/playertd",true) == 0) {
    	...
    	PlayerTestText1 = CreatePlayerTextDraw(playerid, 320.0, 100.0, "Test Text 1"); // Text is txdfile:texture
	...
	if(strcmp(cmd, "/alhamdown", true) == 0) {
		SetPlayerPos(playerid, 1853.9460, -1662.3022, 24.5295);
		return 1;
	}
	...
	if(strcmp(cmd, "/crptex",true) == 0) {
		...
		while(lp != 64) {
			...
			if(lp % 2 == 0) {
				SetPlayerObjectMaterial(playerid, test_tex_objects[lp], 0, 19325, "lsmall_shops", "lsmall_window01", 0xFF00FF00);
			} else {
