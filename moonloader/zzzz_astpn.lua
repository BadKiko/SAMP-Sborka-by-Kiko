...
function main()
    ...
    sampAddChatMessage("{464446}[AsTp{850AB9}Checkpoint{464446}]:{850AB9} Телепорт для АРЗ/Evolve загружен (от MrCreepTon | vk.com/hackmysoftware | thx Gustav Erbaev)")
    sampRegisterChatCommand('astpch', function()
        ...
        if mode ~= 0 then mode = 0 return sampAddChatMessage("{464446}[AsTp{850AB9}Checkpoint{464446}]:{850AB9} Выключен") end
        found_car = false
    ...
    sampRegisterChatCommand('astpnch', function()
        if state then return sampAddChatMessage("{464446}[AsTp{850AB9}Checkpoint{464446}]:{850AB9} Сейчас использовать команду невозможно (возможно вы уже телепортируетесь)") end
        if mode ~= 0 then mode = 0 return sampAddChatMessage("{464446}[AsTp{850AB9}Checkpoint{464446}]:{850AB9} Выключен") end
        found_car = false
...
function ev.onSendPlayerSync(data)
    ...
    if state then
        ...
        if coord then
            if teleporting then
                sync.position = {x = mx, y = my, z = mz}
                sync.send()
                return false
            else
                data.position = {x = mx, y = my, z = mz}
                data.moveSpeed = {x = 0.899999, y = 0.899999, z = -0.1}
            end
        else
            if teleporting then
...
function ev.onSetPlayerPos()
    if coord then return false end
end
