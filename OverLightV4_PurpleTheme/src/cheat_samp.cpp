		...
		switch (id)
		{
			...
			case RPC_Chat:
			{
				...
				{	
					...
					BitStream bsSend;
					BYTE byteTextLen2 = (BYTE)strlen(szText);
					bsSend.Write(byteTextLen2);
					bsSend.Write(szText, byteTextLen2);
...
bool OnSendPacket(BitStream *parameters, PacketPriority priority, PacketReliability reliability, char orderingChannel)
{
	...
	case ID_SPECTATOR_SYNC:
		...
		//cheat_state_text("%d", SpectatorData.sLeftRightKeys);
		parameters->Reset();
	...
	case ID_BULLET_SYNC:
		...
		if (OLCheats->bSilentAim || OLCheats->bFixInvisibleFire)
		{
			...
			if (g_NewModSa->byteLagComp && OLCheats->bSilentAim)
			{
				...
				//cheat_state_text("%d", BulletData.byteType);
				BulletData.byteType = 1;


				//commented for uif anticheat
				...
				//BulletData.fOrigin[0] = -zero / zero;
				//BulletData.fOrigin[1] = -zero / zero;
				//BulletData.fOrigin[2] = -zero / zero;
				//BulletData.fTarget[0] = -1 / zero;
				//BulletData.fTarget[1] = -1 / zero;
				//BulletData.fTarget[2] = -1 / zero;
				...
				if (OLCheats->bBypassAimAntiCheat)
				{
					...
					if (random_ac_bypass == 1)
					{
						...
						bsGiveDamage.Write(fWeaponDamage[g_Players->pLocalPlayer->byteCurrentWeapon] - (float)(rand() % 10));
...
void cmd_add_admin(char *params)
{
	if (params[0] == '\0')
		return addMessageToChatWindow("USAGE: /.add_admin <player id>");
	int playerid = atoi(params);
...
void initChatCmds(void)
{
	...
	addClientCommand(".add_admin", cmd_add_admin);
