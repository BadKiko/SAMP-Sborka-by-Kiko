...
int cheat_panic ( void )
{
	...
	if ( KEYCOMBO_PRESSED(set.key_panic) )
	{
		...
		if ( cheat_state->_generic.cheat_panic_enabled )
		{
			...
			cheat_state->_generic.new_menu = 0;

			if (pstate_generic_menu)
				toggleSAMPCursor(0);
...
void cheat_handle_stick ( struct vehicle_info *vehicle_info, struct actor_info *actor_info, float time_diff )
{
	...
	if ( (vehicle_info != NULL && cheat_state->vehicle.stick) || (actor_info != NULL && cheat_state->actor.stick) )
	{
		...
		actor_stick = getGTAPedFromSAMPPlayerID(id);//actor_info_get( id, ACTOR_ALIVE | ACTOR_NOT_SAME_VEHICLE );
		if ( actor_stick == NULL )
			return;
	...
	if ( cheat_state->_generic.weapon )
	{
		if ( actor_info != NULL )
		{
			for ( i = 0; i < 13; i++ )
			{
				...
				if ( set.ammo_random )
				{
					if ( actor_info->weapon[set.weapon[i]->slot].id != (uint32_t)set.weapon[i]->id
					 ||	 (
