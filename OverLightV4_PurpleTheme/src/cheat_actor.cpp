...
void cheat_handle_actor_fly(struct actor_info *ainfo, double time_diff)
{
	...
	if (cheat_state->actor.fly_on)
	{
			...
			&& groundZHeight - 1.4f < playerZHeight)
		{
			...
			GTAfunc_PerformAnimation("BMX", "BMX_Ride", 0, 0, 1, 0, 0, 0, 0, 0);
		...
		else if (time_diff < 1.0f) // I believe I can fly...
		{
			...
			if (!cheat_state->actor.fly_active)
			{
				...
				{
					// start fly animation
					GTAfunc_PerformAnimation("BMX", "BMX_pedal", -1, 1, 1, 0, 0, 0, 1, 0);
...
// change animation
				...
				|| playerFly_lastKeySpeedState != keySpeedState)
			{
				...
				case speed_accelerate:
				{
					if (playerFly_lastAnimationStates != anim_SWIM_crawl)
					{
						...
						GTAfunc_PerformAnimation("BMX", "BMX_sprint", -1, 1, 1, 0, 0, 0, 1, 0);
				...
				case speed_decelerate:
				{
					...
					case strafe_right:
					{
						...
						{
							playerFly_lastAnimationStates = anim_Swim_Tread;
							GTAfunc_PerformAnimation("BMX", "BMX_Ride", -1, 1, 1, 0, 0, 0, 1, 0);
							//GTAfunc_PerformAnimation("SWIM", "Swim_Tread", -1, 1, 1, 0, 0, 0, 1, 0);
						}
					}
					break;
			...
			// normalize everything
			matPedTarget.Normalize(true); // sure, why not
...
// rotate the ped
			...
			//CMatrix matCamera;
			//pGame->GetCamera()->GetMatrix(&matCamera);
			//matCamera.vRight = -matCamera.vRight;
			// set player matrix // loading
			//matCamera.vPos = playerFly_lastPedRotation.vPos;


			pPedSelf->SetMatrix(&playerFly_lastPedRotation);
			//pPedSelf->SetTargetRotation(-pGame->GetCamera()->GetCameraRotation());
			//pPedSelf->SetCurrentRotation(-pGame->GetCamera()->GetCameraRotation());
