...
int lastRPC = 0;
int lastPacket = 0;
int iStreamedVehicleCount = 0;
int iLastPackedSent = 0;
void traceLastFunc ( const char *szFunc, ...)
{
	_snprintf_s( g_szLastFunc, sizeof(g_szLastFunc)-1, szFunc );
}

void traceLastFunc2(const char *szFunc)
{
	_snprintf_s(g_szLastFunc2, sizeof(g_szLastFunc2) - 1, szFunc);
}
...
void LogChatbox ( bool bLast, const char *fmt, ... )
{
	...
	//wcstombs(cText, wcText, wcslen(wcText));
	//traceLastFunc("wcstombs finish");
	va_start(ap, fmt);
	...
	//std::string str(fmt, strlen(fmt));
	////utf8::replace_invalid(fmt, fmt, fmt);
	////std::string fmt_string = std::string(fmt);
...
LONG WINAPI unhandledExceptionFilter ( struct _EXCEPTION_POINTERS *ExceptionInfo )
{
	...
	Log("Bot Thread: %s", g_szLastFunc2);
