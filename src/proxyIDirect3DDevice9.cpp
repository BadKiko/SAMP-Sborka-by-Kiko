...
char*					rawLogoPNG_Data;
long					rawLogoPNG_Size;

IDirect3DTexture9		*tLogoPNG = NULL;
ID3DXSprite				*sLogoPNG = NULL;
...
//pD3DFontConsoleWnd = console window
...
int						iLastFrame = GetTickCount();

CCheatConsole			*g_pCheatConsole;
...
void LoadSpriteTexture ( void )
{
	...
	snprintf( filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, M0D_TEX_FILE );
...
void RenderVehicleHPBar ( void )
{
	...
	render->D3DBoxi( 0, bottom - fontHeight, hp, fontHeight, D3DCOLOR_ARGB(127, 191, 0, 0), 1000 );
	...
	render->D3DBoxi( 0, bottom - (fontHeight*2), (int)pinfo->hitpoints, fontHeight, D3DCOLOR_ARGB(127, 191, 0, 0), 100 );
	if ( pinfo->armor != NULL )
	...
	if ( !cheat_state->vehicle.air_brake )
	{
		...
		render->D3DBoxi( 101.0f, bottom - fontHeight, 101, 10, D3DCOLOR_ARGB(127, 0, 0, 0), NULL );
		render->D3DBoxi( 101.0f, bottom - fontHeight, (int)(spood * 64), 8, D3DCOLOR_ARGB(127, 70, 170, 255), 100 );
		_snprintf_s( text, sizeof(text)-1, "%0.2f km/h", (float)(spood * 170) );
		pD3DFontFixed->PrintShadow( (float)(2) + 101.0f, (float)(bottom - fontHeight - 1), D3DCOLOR_XRGB(255, 255, 255), text );
	}
	...
	if ( cheat_state->vehicle.air_brake )
	{
		...
		render->D3DBoxi( 101.0f, bottom - fontHeight, 101, 10, D3DCOLOR_ARGB(127, 0, 0, 0), NULL );
		render->D3DBoxi( 101.0f, bottom - fontHeight, (int)(speed_dist * 1.3f), 8, D3DCOLOR_ARGB(127, 70, 170, 255), 100 );
	...
	// acceleration meter
	render->D3DBoxi( 101.0f, bottom - (fontHeight*2) + 1, 101, 10, D3DCOLOR_ARGB(127, 0, 0, 0), NULL );

	if ( speed_acceleration <= -0.01f )
		render->D3DBoxi( 101.0f, bottom - (fontHeight*2) + 1, (int)(speed_acceleration * -4.8f), 8, D3DCOLOR_ARGB(127, 70, 170, 255), 100 );
	else
		render->D3DBoxi( 101.0f, bottom - (fontHeight*2) + 1, (int)(speed_acceleration * 4.8f), 8, D3DCOLOR_ARGB(127, 70, 170, 255), 100 );

	_snprintf_s( text, sizeof(text)-1, "%0.2f m/ss", (float)(speed_acceleration) );
	pD3DFontFixed->PrintShadow( (float)(2) + 101.0f, (float)(bottom - (fontHeight*2) - 1), D3DCOLOR_XRGB(255, 255, 255), text );
}
...
void renderPlayerTags ( void )
{
	...
	bool bIsCheatConsoleOpen = 0;
	
	if ( g_pCheatConsole )
		bIsCheatConsoleOpen = g_pCheatConsole->IsEnabled();
	...
	float		ESP_tag_player_posOffsetZ = 1.25f;
	...
	while ( iter.pos < iter.end )
	{
		...
		D3DCOLOR	color = D3DCOLOR_ARGB( 130, 70, 170, 255 );
		if ( vh > 100.0f )
			vh = 100.0f;
		if ( vh < 100.0f && vh > 60.0f )
			color = D3DCOLOR_ARGB( 120, 70, 170, 255 );
		if ( vh < 60.0f && vh > 20.0f )
			color = D3DCOLOR_ARGB( 110, 70, 170, 255 );
		if ( vh < 20.0f && vh > 0.0f )
			color = D3DCOLOR_ARGB( 100, 70, 170, 255 );
		...
		{
			switch ( getPlayerState(iSAMPID) )
			{
				case PLAYER_STATE_NONE:
					_snprintf_s( statebuf, sizeof(statebuf)-1, "N");
					break;

				case PLAYER_STATE_ONFOOT:
					_snprintf_s( statebuf, sizeof(statebuf)-1, "O");
					break;

				case PLAYER_STATE_DRIVER:
					_snprintf_s( statebuf, sizeof(statebuf)-1, "D");
					break;

				case PLAYER_STATE_PASSENGER:
					_snprintf_s( statebuf, sizeof(statebuf)-1, "P");
					break;

				case PLAYER_STATE_WASTED:
					_snprintf_s( statebuf, sizeof(statebuf)-1, "W");
					break;

				case PLAYER_STATE_SPAWNED:
					_snprintf_s( statebuf, sizeof(statebuf)-1, "S");
					break;
			}

			_snprintf_s( buf, sizeof(buf)-1, "H: %d, A: %d, %s",
			...
			pD3DFontFixedSmall->Print( fPlayerNameTagX, playerBaseY - h + 10.0f, D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf );
			...
			fPlayerNameTagX = g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX/2 + 50.0f - pD3DFont_sampStuff->DrawLength(buf)/2;
			pD3DFont_sampStuff->Print( fPlayerNameTagX, playerBaseY - h - 2, samp_color_get( iSAMPID, 0xDD000000 ), buf );

			if(getPlayerState(iSAMPID) == PLAYER_STATE_DRIVER)
			{
				uint16_t sVehicleID = g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.sVehicleID;

				if(ValidVehicle(sVehicleID))
				{
					D3DCOLOR	color = D3DCOLOR_ARGB( 75, 0, 200, 0 );

					float fVehicleHealth = g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.fVehicleHealth;

					if ( fVehicleHealth > 1000.0f )
						fVehicleHealth = 1000.0f;

					if ( fVehicleHealth < 1000.0f && fVehicleHealth > 650.0f )
						color = D3DCOLOR_ARGB( 111, 0, 200, 0 );

					if ( fVehicleHealth < 650.0f && fVehicleHealth > 350.0f )
						color = D3DCOLOR_ARGB( 111, 200, 200, 0 );

					if ( fVehicleHealth < 350.0f && fVehicleHealth > 0.0f )
						color = D3DCOLOR_ARGB( 111, 200, 0, 0 );

					render->D3DBox( g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX,
									playerBaseY - h + 10.0f + 8.0f + 5.0f + 3.0f, 100.0f, 7.0f, D3DCOLOR_ARGB(111, 0, 0, 0) );

					render->D3DBox( g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
									playerBaseY - h + 10.0f + 8.0f + 5.0f + 4.0f, (fVehicleHealth/10) - 2.0f, 5.0f, color );
				}
			}
			...
			if ( g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2 )
			{
				...
				float w = pD3DFontFixedSmall->DrawLength( statebuf );
				h = pD3DFontFixedSmall->DrawHeight() + 1;
				
				fPlayerNameTagX = g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX/2 + 50.0f - pD3DFont_sampStuff->DrawLength(statebuf)/2;
				...
				render->D3DBox( fPlayerNameTagX, fAFKTextY + 1.0f, pD3DFont_sampStuff->DrawLength( statebuf ) + 2.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0) );
				pD3DFontFixedSmall->Print( fPlayerNameTagX + 1.0f, fAFKTextY, D3DCOLOR_ARGB(130, 170, 170, 170), statebuf );
			}

			if ( set.infoboxes_near_playertags )
			{
				float h = 13;

				render->D3DBox( g_playerTagInfo[iGTAID].tagPosition.fX + 110.0f,
					playerBaseY - 1.0f, 191.0f, 82.0f, samp_color_get(iSAMPID, 0x29000000) );

				render->D3DBoxBorder( g_playerTagInfo[iGTAID].tagPosition.fX + 110.0f,
					playerBaseY - 1.0f, 191.0f, 82.0f, D3DCOLOR_ARGB(191, 0, 0, 0), NULL );

				_snprintf_s( buf, sizeof(buf)-1, "Health: %d, Armour: %d",
					(int)g_Players->pRemotePlayer[iSAMPID]->pPlayerData->fActorHealth,
					(int)g_Players->pRemotePlayer[iSAMPID]->pPlayerData->fActorArmor );

				pD3DFontFixedSmall->Print( g_playerTagInfo[iGTAID].tagPosition.fX + 115.0f, playerBaseY - h + 13.0f, D3DCOLOR_ARGB(191, 255, 255, 255), buf ); // + 13

				_snprintf_s( buf, sizeof(buf)-1, "Ping: %d, Score: %d",
					g_Players->pRemotePlayer[iSAMPID]->iPing, g_Players->pRemotePlayer[iSAMPID]->iScore );

				pD3DFontFixedSmall->Print( g_playerTagInfo[iGTAID].tagPosition.fX + 115.0f, playerBaseY - h + 26.0f, D3DCOLOR_ARGB(191, 255, 255, 255), buf );

				_snprintf_s( buf, sizeof(buf)-1, "Speed: %0.2f km/h",
					(float)(vect3_length(g_Players->pRemotePlayer[iSAMPID]->pPlayerData->onFootData.fMoveSpeed) * 170) );

				pD3DFontFixedSmall->Print( g_playerTagInfo[iGTAID].tagPosition.fX + 115.0f, playerBaseY - h + 39.0f, D3DCOLOR_ARGB(191, 255, 255, 255), buf );
				...
				_snprintf_s( buf, sizeof(buf)-1, "Distance: %0.2f, Weapon ID: %d",
					vect3_dist((float *)fPos, &self->base.matrix[4 * 3]), (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.sVehicleID == NULL) ?
					g_Players->pRemotePlayer[iSAMPID]->pPlayerData->onFootData.byteCurrentWeapon : 
					g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.byteCurrentWeapon );

				pD3DFontFixedSmall->Print( g_playerTagInfo[iGTAID].tagPosition.fX + 115.0f, playerBaseY - h + 52.0f, D3DCOLOR_ARGB(191, 255, 255, 255), buf );

				_snprintf_s( buf, sizeof(buf)-1, "Special Action ID: %d", g_Players->pRemotePlayer[iSAMPID]->pPlayerData->byteSpecialAction );

				pD3DFontFixedSmall->Print( g_playerTagInfo[iGTAID].tagPosition.fX + 115.0f, playerBaseY - h + 65.0f, D3DCOLOR_ARGB(191, 255, 255, 255), buf );

				int iState = getPlayerState(iSAMPID);

				if(iState == PLAYER_STATE_NONE)
					_snprintf_s( buf, sizeof(buf)-1, "State: None" );
				else if(iState ==  PLAYER_STATE_ONFOOT)
					_snprintf_s( buf, sizeof(buf)-1, "State: On Foot");
				else if(iState == PLAYER_STATE_DRIVER)
					_snprintf_s( buf, sizeof(buf)-1, "State: In Vehicle (SAMP ID: %d)", g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.sVehicleID );
				else if(iState == PLAYER_STATE_PASSENGER)
					_snprintf_s( buf, sizeof(buf)-1, "State: Passenger (SAMP ID: %d)", g_Players->pRemotePlayer[iSAMPID]->pPlayerData->passengerData.sVehicleID );
				else if(iState == PLAYER_STATE_WASTED)
					_snprintf_s( buf, sizeof(buf)-1, "State: Wasted" );
				else if ( iState != PLAYER_STATE_SPAWNED )
					_snprintf_s( buf, sizeof(buf)-1, "State: Spawned" );
				else
					_snprintf_s( buf, sizeof(buf)-1, "State: Unknown" );

				pD3DFontFixedSmall->Print( g_playerTagInfo[iGTAID].tagPosition.fX + 115.0f, playerBaseY - h + 78.0f, D3DCOLOR_ARGB(191, 255, 255, 255), buf );
			}
		}
	}
...
void renderVehicleTags ( void )
{
	...
	if ( g_SAMP && g_dwSAMP_Addr && g_pCheatConsole )
	{
...
void renderNetGameInfo ()
{
	...
	if(!bUnkState)
		pD3DFont->Print( 1.0f, (float)(pPresentParam.BackBufferHeight) - pD3DFont->DrawHeight() - 22 - 2, D3DCOLOR_ARGB( 191, 255, 255, 255 ), szInfo );

	CTaskManager *taskManager = pPedSelf->GetPedIntelligence()->GetTaskManager();
	CTask* simplestActiveTask = taskManager->GetSimplestActiveTask();
}
...
void renderScoreList ()
{
	...
	*(int *)( *(DWORD *)( g_dwSAMP_Addr + SAMP_SCOREBOARD_INFO ) ) = 0;
	...
	for ( int i = current_player_id; i < SAMP_PLAYER_MAX; i++ )
	{
		...
		loc[0] += ( pPresentParam.BackBufferWidth / 4 ) + 35.0f;
...
void renderNewCheatInfo ( void )
{
	traceLastFunc( "renderNewCheatInfo()" );
	...
	/*
		Render iTroll target info
	*/

	if(cheat_state->_generic.itroll != 0 && cheat_state->_generic.itroll_target != 0xFFFF && cheat_state->state == CHEAT_STATE_VEHICLE)
	{
		int iTargetPlayerID = cheat_state->_generic.itroll_target;
			
		if(ValidPlayer(iTargetPlayerID) && getPlayerState(iTargetPlayerID) == PLAYER_STATE_DRIVER)
		{
			...
			if(ValidVehicle(sTargetVehicleID))
			{
				...
				if(getPlayerPos(iTargetPlayerID, fPlayerPos))
					Render3DTextLabel("< iTroll Target >", fPlayerPos, samp_color_get(iTargetPlayerID));
			}
		}
	}
}
...
void renderChat ( void )
{
	...
	static bool bLastCheatConsoleState = 0;

	bool bCheatConsoleEnabled = 0;

	bool bJustEnabledCheatConsole = 0;
	bool bJustDisabledCheatConsole = 0;

	if ( g_pCheatConsole )
		bCheatConsoleEnabled = g_pCheatConsole->IsEnabled();

	if ( bLastCheatConsoleState != bCheatConsoleEnabled )
	{
		if ( bLastCheatConsoleState )
			bJustDisabledCheatConsole = 1;
		else
			bJustEnabledCheatConsole = 1;
	}

	bLastCheatConsoleState = bCheatConsoleEnabled;

	static int	state_chatWindow = 0;
	if ( ( KEY_PRESSED(VK_TAB) && set.d3dtext_score ) || bJustEnabledCheatConsole )
	{
	...
	if ( ( KEY_RELEASED(VK_TAB) && set.d3dtext_score ) || bJustDisabledCheatConsole )
	{
...
//#ifdef M0D_DEV_ADVANCEDINFO
		...
		else if ( vinfo != NULL )
		{
			if ( vinfo->passengers[0] == self )
			{
				...
				if ( g_Players->pLocalPlayer->inCarData.sTrailerID != 0 )
				{
					...
					pD3DFontFixed->PrintShadow( 20.0f, y, color, buf );
					( y ) += 1.0f + pD3DFontFixed->DrawHeight();
					sprintf( buf, "    pLocalPlayer->trailerData.vecPosition: %0.2f %0.2f %0.2f",
							 g_Players->pLocalPlayer->trailerData.vecPosition[0],
							 g_Players->pLocalPlayer->trailerData.vecPosition[1],
							 g_Players->pLocalPlayer->trailerData.vecPosition[2] );
					pD3DFontFixed->PrintShadow( 20.0f, y, color, buf );
					( y ) += 1.0f + pD3DFontFixed->DrawHeight();
					sprintf( buf, "    pLocalPlayer->trailerData.vecDirection: %0.2f %0.2f %0.2f",
							 g_Players->pLocalPlayer->trailerData.vecDirection[0],
							 g_Players->pLocalPlayer->trailerData.vecDirection[1],
							 g_Players->pLocalPlayer->trailerData.vecDirection[2] );
					pD3DFontFixed->PrintShadow( 20.0f, y, color, buf );
					( y ) += 1.0f + pD3DFontFixed->DrawHeight();
					...
					pD3DFontFixed->PrintShadow( 20.0f, y, color, buf );
					( y ) += 1.0f + pD3DFontFixed->DrawHeight();
					sprintf( buf, "    pLocalPlayer->trailerData.vecVelocity: %0.2f %0.2f %0.2f",
							 g_Players->pLocalPlayer->trailerData.vecVelocity[0],
							 g_Players->pLocalPlayer->trailerData.vecVelocity[1],
							 g_Players->pLocalPlayer->trailerData.vecVelocity[2] );
					pD3DFontFixed->PrintShadow( 20.0f, y, color, buf );
					( y ) += 1.0f + pD3DFontFixed->DrawHeight();
				}
		...
		else
		{
			...
			pD3DFontFixed->PrintShadow( 20.0f, y, color, buf );
			( y ) += 1.0f + pD3DFontFixed->DrawHeight();
			sprintf( buf, "    pLocalPlayer->onFootData.iCurrentAnimationID: %d",
		...
		sprintf(buf, "    pLocalPlayer->aimData.bUnk: %u", g_Players->pLocalPlayer->aimData.bUnk);
		pD3DFontFixed->PrintShadow( 20.0f, y, color, buf );
		( y ) += 1.0f + pD3DFontFixed->DrawHeight();
...
void renderSAMP ( void )
{
	...
	if ( !g_renderSAMP_initSAMPstructs )
	{
		...
		// initialize raknet
		g_iRakClient = new iRakClient( g_SAMP->pRakClientInterface );
		...
		*(BYTE*)0xBAB318 = 0;  *(BYTE*)0x53E94C = 0;
		...
		// Initialize CheatConsole
		if ( !g_pCheatConsole )
			g_pCheatConsole = new CCheatConsole();

		addMessageToConsole("Console successfully loaded.");
	...
	if ( g_SAMP != NULL )
	{
		...
		if ( cheat_state->_generic.nofall && (g_Players-> pLocalPlayer-> onFootData.sCurrentAnimationID == 0x469 || g_Players-> pLocalPlayer-> onFootData.sCurrentAnimationID == 0x4B8) )
...
void LoadLogoFileIntoMemory()
{
	CHAR filename[MAX_PATH];

	snprintf( filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, M0D_LOGO_FILE );

	FILE * LogoFile = fopen(filename, "rb");
	size_t Result;

	if(LogoFile == NULL)
	{
		Log("Failed to load the logo, so it won't appear in the menu. :'(");
		return;
	}

	fseek (LogoFile , 0 , SEEK_END);
	rawLogoPNG_Size = ftell(LogoFile);
	rewind(LogoFile);

	rawLogoPNG_Data = (char*) malloc (sizeof(char)*rawLogoPNG_Size);

	if(rawLogoPNG_Data == NULL)
		return;

	Result = fread(rawLogoPNG_Data, 1, rawLogoPNG_Size, LogoFile);

	if (Result != rawLogoPNG_Size)
		return;
...
void texturesInitResources ( IDirect3DDevice9 *pDevice, D3DPRESENT_PARAMETERS *pPresentationParameters )
{
	...
	if(!tLogoPNG)
	{
		switch(D3DXCreateTextureFromFileInMemoryEx(pDevice, rawLogoPNG_Data, rawLogoPNG_Size,  260, 110, 0, NULL, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_FILTER_NONE, 0xFF000000, NULL, NULL, &tLogoPNG))
		{
			case D3D_OK:
				break;

			case D3DERR_NOTAVAILABLE:
				Log("D3DXCreateTextureFromFileInMemoryEx failed: D3DERR_NOTAVAILABLE.");
				break;

			case D3DERR_OUTOFVIDEOMEMORY:
				Log("D3DXCreateTextureFromFileInMemoryEx failed: D3DERR_OUTOFVIDEOMEMORY.");
				break;

			case D3DERR_INVALIDCALL:
				Log("D3DXCreateTextureFromFileInMemoryEx failed: D3DERR_INVALIDCALL.");
				break;

			case D3DXERR_INVALIDDATA:
				Log("D3DXCreateTextureFromFileInMemoryEx failed: D3DXERR_INVALIDDATA.");
				break;

			case E_OUTOFMEMORY:
				Log("D3DXCreateTextureFromFileInMemoryEx failed: E_OUTOFMEMORY.");
				break;

			default:
				Log("D3DXCreateTextureFromFileInMemoryEx failed: Unknown error.");
				break;
		}
	}
	*/
...
void proxyID3DDevice9_UnInitOurShit ( void )
{
	...
	pD3DFontConsoleWnd->Invalidate();

	SAFE_RELEASE( chams_green );
	SAFE_RELEASE( chams_blue );
	SAFE_RELEASE( chams_red );

	if ( sLogoPNG && tLogoPNG )
	{
		SAFE_RELEASE( sLogoPNG );
		SAFE_RELEASE( tLogoPNG );
	}
...
void proxyID3DDevice9_InitOurShit ( D3DPRESENT_PARAMETERS *pPresentationParameters )
{
	...
	pD3DFontConsoleWnd->Initialize( origIDirect3DDevice9 );
	...
	if ( !proxyIDirect3DDevice9_init )
	{
		...
		if ( iIsSAMPSupported )
		{
			...
			if ( set.ant_tweak_menu )
			{
				...
				twBar_MainMenu = TwNewBar("Main_Menu", 0);
				twBar_NewCheats = TwNewBar("New_Cheats", 1);
				twBar_CarOperations = TwNewBar("Car_Operations", 1);
				twBar_Aimbot = TwNewBar("Aimbot", 1);
				twBar_AutoPilot = TwNewBar("AutoPilot", 1);
				twBar_Bots = TwNewBar("Bots", 1);
				twBar_RakNet = TwNewBar("RakNet", 1);
				twBar_Interface = TwNewBar("Interface", 1);
			}
		}
...
#define HUD_TEXT( x, color, text ) \
	pD3DFont->Print( 1.0f + (x), (float)(pPresentParam.BackBufferHeight) - pD3DFont->DrawHeight() - 3, (color), \
						   (text) ); \
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	uint32_t	color_enabled = D3DCOLOR_ARGB( 255, 70, 170, 255 );
	...
	if ( isBeginRenderWIN )
	{
		...
		if ( set.d3dtext_hud )
		{
			if ( cheat_panic() || gta_menu_active() || cheat_state->state == CHEAT_STATE_NONE )
			{
				...
				if( !set.flickering_problem )
				{
					uint32_t	bar_color = D3DCOLOR_ARGB( 145, 70, 170, 255 );
					uint32_t	line_color = D3DCOLOR_ARGB( 155, 255, 255, 255 );
					...
					render->D3DBoxi( (int)x - 1,
									 (int)(pPresentParam.BackBufferHeight - 3) - (int)pD3DFont->DrawHeight() - 3,
									 (int)(pPresentParam.BackBufferWidth + 14), 2, line_color, NULL );

					if ( iIsSAMPSupported )
					{
						_snprintf_s( buf, sizeof(buf)-1, "%s for %s", NAME, g_szSAMPVer );
						HUD_TEXT( x, D3DCOLOR_ARGB(150, 255, 255, 255), buf );
					}
					else
					{
						HUD_TEXT( x, D3DCOLOR_ARGB(150, 255, 255, 255), NAME );
					}

					_snprintf_s( buf, sizeof(buf)-1, "by P3ti, Pabloko");
					pD3DFont->Print( pPresentParam.BackBufferWidth - pD3DFont->DrawLength(buf) - 2, pPresentParam.BackBufferHeight - pD3DFont->DrawHeight() - 2, D3DCOLOR_ARGB(150, 255, 255, 255), buf );					
				}
			...
			else
			{
				...
				if ( set.hud_draw_bar )
				{
					...
					render->D3DBoxi( (int)x - 1, (int)(pPresentParam.BackBufferHeight) - (int)pD3DFont->DrawHeight() - 3,
									 (int)(pPresentParam.BackBufferWidth + 14), 22, bar_color, NULL );
				}

				if ( set.hud_indicator_inv )
				{
					HUD_TEXT_TGL( x, cheat_state->_generic.hp_cheat ? color_enabled : color_disabled, "Invulnerable" );
				}
				...
				if ( set.hud_indicator_nocollision )
				{
					HUD_TEXT_TGL( x, cheat_state->_generic.nocols_actors_enabled || cheat_state->_generic.nocols_vehicles_enabled ||
									 cheat_state->_generic.nocols_walls_enabled ? color_enabled : color_disabled, "NoCollision" );
				}
				...
				if ( set.hud_fps_draw )
				{
					...
					if ( m_FPS_int >= 23 )
						color_fps = D3DCOLOR_ARGB( 255, 70, 170, 255 );
					else if ( m_FPS_int >= 13 && m_FPS_int <= 22 )
						color_fps = D3DCOLOR_XRGB( 235, 235, 0 );
					else if ( m_FPS_int <= 12 )
						color_fps = D3DCOLOR_XRGB( 235, 0, 0 );
					...
					float fpsValueTextX = pPresentParam.BackBufferWidth - pD3DFont->DrawLength(buf) - 4;
					float fpsTextX = fpsValueTextX - pD3DFont->DrawLength(fpsText);

					HUD_TEXT( fpsTextX, color_text, fpsText );
					HUD_TEXT( fpsValueTextX, color_fps, buf );
				}
			}
			...
			if ( cheat_state->state == CHEAT_STATE_VEHICLE )
			{
				...
				if ( set.hud_indicator_inveh_autopilot && g_SAMP != NULL )
				{
					HUD_TEXT_TGL( x, cheat_state->_autopilot.driving ? color_enabled : color_disabled, "AutoPilot" );
				}

				if ( set.hud_indicator_inveh_itroll && g_SAMP != NULL )
				{
					HUD_TEXT_TGL( x, !cheat_state->_generic.itroll ? color_disabled : color_enabled, "iTroll" );
				}
			...
			else if ( cheat_state->state == CHEAT_STATE_ACTOR )
			{
				...
				if ( set.hud_indicator_onfoot_aimbot && iIsSAMPSupported )
				{
					HUD_TEXT_TGL( x, cheat_state->_aimbot.AimbotToggle ? color_enabled : color_disabled, "Aimbot" );
				}
				...
				if ( set.hud_indicator_onfoot_fly )
				{
					if ( set.use_bossfly )
					{
						HUD_TEXT_TGL( x, cheat_state->actor.fly_on ? color_enabled : color_disabled, "BossFly" );
					}
					else
					{
				...
				if ( set.hud_indicator_onfoot_maprun )
				{
					HUD_TEXT_TGL( x, cheat_state->_generic.maprun ? color_enabled : color_disabled, "MapRun" );
				}
		...
		// render new menu
		if ( !set.ant_tweak_menu && iIsSAMPSupported )
		{
													  ...
													  UINT primCount )
{
	if ( !cheat_state->_generic.cheat_panic_enabled )
	{
		...
		{
			if (set.wallhack_on) origIDirect3DDevice9->SetRenderState( D3DRS_ZENABLE, false );
			...
			if (set.wallhack_on) origIDirect3DDevice9->SetRenderState( D3DRS_PATCHEDGESTYLE, D3DPATCHEDGE_CONTINUOUS );

			if (set.chams_on && set.wallhack_on) origIDirect3DDevice9->SetPixelShader( chams_blue );
			
			if (set.wallhack_on) origIDirect3DDevice9->DrawIndexedPrimitive( PrimitiveType, BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount );

			if (set.chams_on) origIDirect3DDevice9->SetPixelShader( chams_green );

			if (set.wallhack_on) origIDirect3DDevice9->SetRenderState( D3DRS_ZENABLE, true );
...
void Render3DTextLabel(char* text, float pos[3], DWORD color)
{
	float screenpos[3];
	D3DXVECTOR3 poss, screenposs;
	...
	pD3DFontFixed->PrintShadow( screenpos[0], screenpos[1] - 5.0f, color, text );
}