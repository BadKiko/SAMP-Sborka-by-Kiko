...
//@Pabloko: add a minimize-maximize methods (compatibility)

	static int TwMinimizeBar(TwBar *bar)
	{
		int visible = 1;
		return TwSetParam(bar, NULL, "iconified", TW_PARAM_INT32, 1, &visible);
	}

	static int TwIsBarMinimized(TwBar *bar)
	{
		int visible;
		TwGetParam(bar, NULL, "iconified", TW_PARAM_INT32, 1, &visible);
		return visible;
	}

	static int TwMaximizeBar(TwBar *bar)
	{
		int visible = 0;
		return TwSetParam(bar, NULL, "iconified", TW_PARAM_INT32, 1, &visible);
	}

	static TwBar *  TwNewBar(const char *barName, bool minimized)
	{
		TwBar * tb = TwNewBar(barName);
		if (minimized)
			TwMinimizeBar(tb);
		else
			TwMaximizeBar(tb);
	...
	static TwBar *  TwNewBar(const char *barName, bool minimized, char * helpstring)
	{
		TwBar * tb = TwNewBar(barName, minimized);
		CString help = CString(helpstring);
		TwSetParam(tb, NULL, "help", TW_PARAM_CSTRING, 1, help);
		return tb;
	}
