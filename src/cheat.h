...
// autopilot destination pool sizes
#define AUTOPILOT_DEST_MAX	200
...
struct cheat_packet_dsbl
{
	bool DSBL_SND_PLAYER_SYNC;
	bool DSBL_SND_UNOCCUPIED_SYNC;
	bool DSBL_SND_TRAILER_SYNC;
	bool DSBL_SND_PASSENGER_SYNC;
	bool DSBL_SND_SPECTATOR_SYNC;
	bool DSBL_SND_AIM_SYNC;
	bool DSBL_SND_VEHICLE_SYNC;
	bool DSBL_SND_WEAPONS_UPDATE;
	bool DSBL_SND_STATS_UPDATE;
	bool DSBL_SND_BULLET_SYNC;

	bool DSBL_RCV_PLAYER_SYNC;
	bool DSBL_RCV_MARKERS_SYNC;
	bool DSBL_RCV_UNOCCUPIED_SYNC;
	bool DSBL_RCV_TRAILER_SYNC;
	bool DSBL_RCV_PASSENGER_SYNC;
	bool DSBL_RCV_AIM_SYNC;
	bool DSBL_RCV_VEHICLE_SYNC;
	bool DSBL_RCV_BULLET_SYNC;
};
...
struct cheat_state_generic
{
	...
	int		nocols_actors_enabled;		/* player collision */
	int		nocols_vehicles_enabled;	/* vehicle collision */
	...
	int		fakebullets;
	int		bulletbugger;
	int		playerbugger_onfoot;
	int		playerbugger_onfoot_targetid;
	int		playerbugger_incar;
	int		antiafk;
	int		antiafk_move;
	...
	int		serverlagger;
	int		modified_packet_attack;

	int		blockdialogs;

	int		car_remote_control;
	...
	int		anti_vehcomponentcrasher;
	int		anti_bulletcrasher;
	int		anti_weaponcrasher;
	int		anti_traincrasher;
	int		anti_attachedobjectcrasher;

	int		prevent_trolling_vehicle;
...
struct cheat_state_samp
{
};

struct cheat_state_aimbot
{
	int AimbotToggle;

	//AIM CHEATS
	int playerAimed; //LAS PLAYER AIMED WITH GREEN TRIANGLE
	int RealAIMID; //ID OF AIMED
	int TypeAIMID; //1 PED 2 VEHICLE
	float DistanceAIM; //DISTANCE
	bool CurrentAIMstreamed; //IS STREAMED?
	bool playerAiming; //IM AIMING?
	float playerAimingPos[3]; //POS OF PLAYER AIMED
	bool silentAIMsafe; //IS PLAYER IN TARGET OF SAFE CROSSHAIR MARGINS?
	bool carAimbot; 
	bool skinAimbot;
	bool silentAimbot; //EXECUTION MOD
	bool sniperAimbot;
	bool silentFrame; //SHOW FRAME WHEN AIM
	float silentFrameX; 
	float silentFrameY;
	bool executionMode; //IDK
};

struct cheat_state_autopilot
{
	bool driving;
	bool apdriving;
};
...
struct cheat_state
{
	...
	struct cheat_state_samp			_samp;
	struct cheat_state_aimbot		_aimbot;
	struct cheat_state_autopilot	_autopilot;
	...
	struct cheat_packet_dsbl		packet_dsbl_settings;
};
