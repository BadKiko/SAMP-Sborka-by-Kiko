...
bool RakPeer::RPC( int* uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp )
{
#ifdef _DEBUG
...
#pragma warning( disable : 4701 ) // warning C4701: local variable <variable name> may be used without having been initialized
#endif
bool RakPeer::RPC( int* uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp )
{
	if (bitStream)
		return RPC(uniqueID, (const char*) bitStream->GetData(), bitStream->GetNumberOfBitsUsed(), priority, reliability, orderingChannel, playerId, broadcast, shiftTimestamp);
	else
		return RPC(uniqueID, 0, 0, priority, reliability, orderingChannel, playerId, broadcast, shiftTimestamp);
}
...
bool RakPeer::HandleRPCPacket( const char *data, int length, PlayerID playerId )
{
	...
	if(iProcessedBotID != -1)
	{
		if(g_pBotManager)
		{
			...
			g_pBotManager->HandleRPC(iProcessedBotID, (int)uniqueIdentifier, bs2);
		}
	}
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	// We didn't check this datagram to see if it came from a connected system or not yet.
	// Therefore, this datagram must be under 17 bits - otherwise it may be normal network traffic as the min size for a raknet send is 17 bits
	else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST && length == sizeof(unsigned char)*2+1) // SAMP
	{
		for (i=0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length*8, playerId);
	...
	if ( remoteSystem )
	{
		...
		{
			if ( !(
			( (unsigned char)data[0] == ID_OPEN_CONNECTION_REQUEST && length <= 3 ) || // SAMP
...
//						packet->playerIndex = ( PlayerIndex ) remoteSystemIndex;
					...
					else if ( (unsigned char)(data)[0] == ID_DETECT_LOST_CONNECTIONS && byteSize == sizeof(unsigned char) )
					{
						...
						/* SAMP */ +sizeof(unsigned short)+sizeof(unsigned short))
					{
