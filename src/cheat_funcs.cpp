...
bool hasDamageModel ( unsigned long ulModel )
{
    switch ( GetVehicleType ( ulModel ) )
    {
...
bool hasDoors ( unsigned long ulModel )
{
...
void vehicle_setColor0 ( vehicle_info *vinfo, int new_color )
{
	...
	if ( g_SAMP != NULL )
	{
		...
		if(VehicleID != NULL)
			sendSCMEvent( 3, VehicleID, new_color, vinfo->color[1] );
	}
}
...
void vehicle_setColor1 ( vehicle_info *vinfo, int new_color )
{
	...
	if ( g_SAMP != NULL )
	{
		...
		if(VehicleID != NULL)
			sendSCMEvent( 3, VehicleID, vinfo->color[0], new_color );
	}
}
