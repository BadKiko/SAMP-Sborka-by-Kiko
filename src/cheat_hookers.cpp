...
hk_PlCol_process:
	...
	// No actor collisions
	if ( cheat_state->_generic.nocols_actors_enabled )
	{
		// Player (edi) -> Player (esi)
		if ( !isBadPtr_GTA_pPed((actor_info *)PlayerCollision_edi_back) &&
			(!isBadPtr_GTA_pPed((actor_info *)PlayerCollision_esi_back) ||
			!isBadPtr_GTA_pPed((actor_info *)PlayerCollision_esi_back) ) )
			goto hk_PlCol_noCol;

		// Player (esi) -> Player (edi)
		if ( !isBadPtr_GTA_pPed((actor_info *)PlayerCollision_esi_back) &&
			!isBadPtr_GTA_pPed((actor_info *)PlayerCollision_edi_back) )
			goto hk_PlCol_noCol;
	}

	// No vehicle collisions
	if ( cheat_state->_generic.nocols_vehicles_enabled )
	{
...
bool _cdecl CPed_AddGogglesModelCheck (void* pPedInterface)
{
	...
	if(pPedSelf == pPedInterface)
		return true;

	return false;
}

DWORD CPed_AddGogglesModel_RTN = 0x5E3AD4;
...
void cheat_hookers_installhooks ( void )
{
	...
	HookInstall( 0x5E3ACB, (DWORD)HOOK_CPed_AddGogglesModel, 6 );
}