...
CD3DFont				*pD3DFont_Footer = new CD3DFont("Small Fonts", 8, FCR_BOLD | FCR_BORDER);
...
CD3DFont				*pD3DFont_Menu_Hover = new CD3DFont("Arial", 10, FCR_BOLD);
...
static void mmm_yummy_poop(const void *info, int *enabled, int *prev_enabled, int *_render, const char *teh_name)
{
	if (info == NULL)
	{
		*prev_enabled = -1;
	...
	else
	{
		...
		if (!*prev_enabled && *enabled && *_render)
		{
			*enabled = 0;
			*prev_enabled = 0;
			*_render = 0;
			cheat_state_text("Отключен %s", teh_name);
		}
		else if (*prev_enabled && !*enabled)
		{
			cheat_state_text("Переключено на s0beit %s", teh_name);
			*_render = 1;
		}
		else if (!*prev_enabled && *enabled)
		{
			cheat_state_text("Переключено на SA:MP %s", teh_name);
			*_render = 0;
		}
...
void RenderMenu(void)
{
	...
	float fXdescription = 0, fYdescription = 0;
	...
	render->D3D_OL_BoxRounded(left, y - 10.0f, MENU_WIDTH, MENU_HEIGHT + 15.0f, 6.0f, OL_COLOR4(64));
	...
	for (i = top_pos; i < top_pos + MENU_ROWS; i++, y += ROW_HEIGHT + 3)
	{
		...
		if (i == pos)
		{
			...
			render->D3D_OL_BoxRounded(left + distance + 2.0f, (int)floorf(y) + 1, iWidth - 4.0f, ROW_HEIGHT + 1, 4.0f, OL_COLOR3(127));
		...
		else
		{
			if (item->name[0] == '\t')
			{
				render->D3D_OL_BoxRounded(left + distance + 2.0f, (int)floorf(y) + 1, MENU_WIDTH - 4.0f /*MENU_WIDTH - 1*/, ROW_HEIGHT + 1.0f, 4.0f,  MOD_SA_GROUP_COLOR1(127));
				//distance = -10.0f;
				//render->D3D_OL_Rounded(left, (int)floorf(y) + 1, MENU_WIDTH + ROW_HEIGHT / 4.0f /*MENU_WIDTH - 1*/, ROW_HEIGHT + 1.0f, MOD_SA_GROUP_COLOR1(127));
				//render->D3D_OL_Rounded(left + distance, (int)floorf(y) + 1, MENU_WIDTH - 1 + 10, ROW_HEIGHT + 1, MOD_SA_GROUP_COLOR1(127));
			...
			{
				render->D3D_OL_BoxRounded(left + distance + 2.0f, (int)floorf(y) + 1, MENU_WIDTH - 4.0f/*MENU_WIDTH - 1*/, ROW_HEIGHT + 1.0f, 4.0f, OL_COLOR1_2(127));
			}
		}
		...
		{
			if (item->name[0] == '\t')
				pD3DFont_Menu->Print(item->name, MOD_SA_GROUP_COLOR2(255),
					x + (MENU_WIDTH - pD3DFont_Menu->DrawLength(item->name)) / 2.0f, y, false, false);
			else
				pD3DFont_Menu->Print(item->name, enabled ? OL_COLOR2(255) : OL_COLOR3(255), x + distance + 2.0f, y, false, false);
		}
	...
	if (cheat_state->_generic.teletext)
	{
		for (i = 0; i < TELEPORT_MAX; i++)
		{
			...
			if (vect3_near_zero(pos))
				continue;
			_snprintf_s(buf, sizeof(buf) - 1, "Телепорт %d (%0.1f)", i, vect3_dist(&self->base.matrix[4 * 3], pos));
...
void RenderPedHPBar(void)
{
	...
	_snprintf_s(text, sizeof(text) - 1, "Здоровье: %d", (int)info->hitpoints);
	...
	if (info->armor == NULL)
	{
		_snprintf_s(text, sizeof(text) - 1, "Нет броньи");
	...
	else
	{
		...
		_snprintf_s(text, sizeof(text) - 1, "Бронья: %d", (int)info->armor);
		pD3DFontFixed->PrintShadow((float)(2), (float)(bottom - (fontHeight * 2)), D3DCOLOR_XRGB(255, 255, 255), text);
	}
}
...
void RenderVehicleHPBar(void)
{
	...
	_snprintf_s(text, sizeof(text) - 1, "Здоровье т/с: %d", hp);
	...
	if (!set.speedometer_old_enable)
	{
		...
		_snprintf_s(text, sizeof(text) - 1, "Здоровье персонажа: %d", (int)pinfo->hitpoints);
...
void RenderPickupTexts(void)
{
	...
	if (self != NULL)
	{
		for (i = 0; i < SAMP_MAX_PICKUPS; i++)
		{
			...
			_snprintf_s(buf, sizeof(buf) - 1, "Пикап: %d, ID Модели: %d", i, g_SAMP->pPools->pPickup->pickup[i].iModelID);
...
void RenderObjectTexts(void)
{
	...
	for (i = 0; i < SAMP_MAX_OBJECTS; i++)
	{
		...
		_snprintf_s(buf, sizeof(buf) - 1, "Объект: %d, ID Модели: %d", i,
...
void renderPlayerInfoList(void)
{
	...
	for (i = current_player_id; i < SAMP_MAX_PLAYERS; i++)
	{
		...
		{
			_snprintf_s(buf, sizeof(buf) - 1, "Имя: %s (ID: %d), вне зоны стрима", getPlayerName(i), i);
		...
		case PLAYER_STATE_NONE:
			_snprintf_s(buf, sizeof(buf) - 1, "Имя: %s (ID: %d), Статус: Нет", szPlayerName, i);
			pD3DFont_sampStuff->PrintShadow(width, height, color, buf);
			break;
		...
		case PLAYER_STATE_DRIVER:
			_snprintf_s(buf, sizeof(buf) - 1, "Имя: %s (ID: %d), Статус: За рулем (%s (%d)), Здоровье т/с: %d, Дистанция: %0.2f",
		...
		case PLAYER_STATE_PASSENGER:
			_snprintf_s(buf, sizeof(buf) - 1, "Имя: %s (ID: %d), Статус: Пассажир (%s (%d)), Дистанция: %0.2f",
				szPlayerName, i, vehicle->name, iVehicleID, vect3_dist((float *)pos, &self->base.matrix[4 * 3]));
			pD3DFont_sampStuff->PrintShadow(width, height, color, buf);
			break;

		case PLAYER_STATE_WASTED:
			_snprintf_s(buf, sizeof(buf) - 1, "Имя: %s (ID: %d), Статус: Пойман", szPlayerName, i);
			pD3DFont_sampStuff->PrintShadow(width, height, color, buf);
			break;

		case PLAYER_STATE_SPAWNED:
			_snprintf_s(buf, sizeof(buf) - 1, "Имя: %s (ID: %d), Статус: Заспавнен", szPlayerName, i);
			pD3DFont_sampStuff->PrintShadow(width, height, color, buf);
			break;
		}
...
void NewRenderScoreList()
{
	...
	pD3DFont_sampStuff->PrintShadow(LabelNickPos[0], LabelNickPos[1], D3DCOLOR_ARGB(255, 0, 158, 158), "Ник");
	float LabelScorePos[2] = { pos[0] + 20.0f + width / 2 - pD3DFont_sampStuff->DrawLength("Счет", false) / 2, pos[1] + 20.0f };
	pD3DFont_sampStuff->PrintShadow(LabelScorePos[0], LabelScorePos[1], D3DCOLOR_ARGB(255, 0, 158, 158), "Счет");
	float LabelPingPos[2] = { pos[0] + 20.0f + width - pD3DFont_sampStuff->DrawLength("Пинг", false) - (20.0f * 2), pos[1] + 20.0f };
	pD3DFont_sampStuff->PrintShadow(LabelPingPos[0], LabelPingPos[1], D3DCOLOR_ARGB(255, 0, 158, 158), "Пинг");
	char 	buffer[512];
...
void renderScoreList()
{
	...
	_snprintf_s(buffer, sizeof(buffer) - 1, "%s (%s:%d) Подключенные игроки: %d", g_SAMP->szHostname, g_SAMP->szIP,
	...
	for (int i = current_player_id; i < SAMP_MAX_PLAYERS; i++)
	{
		...
		if (g_Players->pRemotePlayer[i]->iIsNPC == 1)
			_snprintf_s(buffer, sizeof(buffer) - 1, "[БОТ]%s (%d)", getPlayerName(i), i);
		else
...
void clickWarp()
{
	...
	if (GetCursorPos(&cursor_pos) && ScreenToClient(pPresentParam.hDeviceWindow, &cursor_pos))
	{
		...
		if (bCollision && pCollision)
		{
			...
			if (pCollisionEntity && pCollisionEntity->nType == ENTITY_TYPE_VEHICLE)
			{
				...
				if (pCVehicleTeleport)
				{
					...
					{
						sprintf(buf, "Телепортироваться в %s", vehicleEntry->name);
					}
				}
				else
				{
					sprintf(buf, "Дистанция %0.2f", vect3_dist(&vecOrigin.fX, &vecGroundPos.fX));
				}
			}
		...
		if (pCVehicleTeleport != nullptr)
		{
			...
			pD3DFontChat->PrintShadow(vehScreenposs.x, vehScreenposs.y + 10.0f, (DWORD)-1, buf);
		}
...
void renderPlayerInfo(int iPlayerID)
{
	...
	if (iPlayerID == -2)
	{
		pD3DFontFixed->PrintShadow(20.0f, y, color, "Локальная информация об игроке");

		color = D3DCOLOR_ARGB(0xFF, 0x99, 0x99, 0x99);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Имя: %s", getPlayerName(g_Players->sLocalPlayerID));
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();

		sprintf(buf, "ID Команды: %u", g_Players->pLocalPlayer->byteTeamID);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Текущий Интерьер: %u", g_Players->pLocalPlayer->byteCurrentInterior);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();

#ifdef M0D_DEV_ADVANCEDINFO
		sprintf(buf, "Пойман: %i", g_Players->pLocalPlayer->iIsWasted);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Наблюдатель: %i", g_Players->pLocalPlayer->iIsSpectating);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "ID Наблюдателя: %i", g_Players->pLocalPlayer->iSpectateID);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Режим наблюдателя: %u", g_Players->pLocalPlayer->byteSpectateMode);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Тип наблюдателя: %u", g_Players->pLocalPlayer->byteSpectateType);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Запущен режим наблюдателя: %i", g_Players->pLocalPlayer->iInitiatedSpectating);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Тик наблюдателя: %i", g_Players->pLocalPlayer->ulSpectateTick);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Итерации (тик): %i", g_Players->pLocalPlayer->ulSendTick);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "Выбор класса при смерти: %i", g_Players->pLocalPlayer->iClassSelectionOnDeath);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
#endif
		...
		if (vinfo != NULL)
		{
			pD3DFontFixed->PrintShadow(20.0f, y, color, "Транспорт");
			(y) += 1.0f + pD3DFontFixed->DrawHeight();
			if (vinfo->passengers[0] == actor_info_get(ACTOR_SELF, ACTOR_ALIVE))
			{
				sprintf(buf, "ID Транспорта: %u", g_Players->pLocalPlayer->sCurrentVehicleID);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
			}
		}
		...
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		sprintf(buf, "_______ДОПОЛНИТЕЛЬНАЯ_ИНФОРМАЦИЯ________");
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
		...
		{
			sprintf(buf, "    Пассажир: %i", g_Players->pLocalPlayer->iPassengerDriveBy);
			pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
			(y) += 1.0f + pD3DFontFixed->DrawHeight();
			sprintf(buf, "    ID Транспорта: %u",
				g_Players->pLocalPlayer->passengerData.sVehicleID);
			pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
			(y) += 1.0f + pD3DFontFixed->DrawHeight();
			sprintf(buf, "    Позиция: %0.2f %0.2f %0.2f",
		...
		else if (vinfo != NULL)
		{
			...
			{
				sprintf(buf, "    В радиоуправляемой машинке: %u", g_Players->pLocalPlayer->iIsInRCVehicle);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Нажатая кнопка: %i", g_Players->pLocalPlayer->inCarData.sKeys);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Бронья: %u", g_Players->pLocalPlayer->inCarData.byteArmor);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Здоровье: %u",
				...
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Состояние передачи при падении: %u",
					g_Players->pLocalPlayer->inCarData.byteLandingGearState);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Сирена: %u", g_Players->pLocalPlayer->inCarData.byteSiren);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    ID Транспорта: %i", g_Players->pLocalPlayer->inCarData.sVehicleID);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    ID Прицепа: %i",
				...
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Скорость движения: %0.2f %0.2f %0.2f",
				...
				if (g_Players->pLocalPlayer->inCarData.sTrailerID != 0)
				{
					...
					pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
					(y) += 1.0f + pD3DFontFixed->DrawHeight();
					sprintf(buf, "    Поворот (кватернион): %0.2f %0.2f %0.2f %0.2f",
					...
					sprintf(buf, "    Скорость поворота: %0.2f %0.2f %0.2f",
					...
					sprintf(buf, "    Данные прицепа (размер): %d",
		...
		{
			sprintf(buf, "    Бронья: %u", g_Players->pLocalPlayer->onFootData.byteArmor);
			pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
			(y) += 1.0f + pD3DFontFixed->DrawHeight();
			sprintf(buf, "    Здоровье: %u", g_Players->pLocalPlayer->onFootData.byteHealth);
			pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
			(y) += 1.0f + pD3DFontFixed->DrawHeight();
			...
			{
				sprintf(buf, "    Состояние серфа на машине: %i", g_Players->pLocalPlayer->surfData.iSurfMode);
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Позиция где стоит персонаж на машине: %0.2f %0.2f %0.2f",
				...
				pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
				(y) += 1.0f + pD3DFontFixed->DrawHeight();
				sprintf(buf, "    Смещение позиции персонажа на машине: %0.2f %0.2f %0.2f",
			...
			pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
			(y) += 1.0f + pD3DFontFixed->DrawHeight();
			sprintf(buf, "    ID Текущей анимации: %d",
				g_Players->pLocalPlayer->onFootData.sCurrentAnimationID);
			pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
			(y) += 1.0f + pD3DFontFixed->DrawHeight();
			sprintf(buf, "    Флаги Анима (Anim_Flags): %d",
		...
		sprintf(buf, "    Режим камеры: %u", g_Players->pLocalPlayer->aimData.byteCamMode);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();

		sprintf(buf, "    Поворот куда смотрит игрок: %0.2f %0.2f %0.2f", g_Players->pLocalPlayer->aimData.vecAimf1[0], g_Players->pLocalPlayer->aimData.vecAimf1[1], g_Players->pLocalPlayer->aimData.vecAimf1[2]);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();

		sprintf(buf, "    Позиция куда смотрит игрок (Точка пересечения): %0.2f %0.2f %0.2f", g_Players->pLocalPlayer->aimData.vecAimPos[0], g_Players->pLocalPlayer->aimData.vecAimPos[1], g_Players->pLocalPlayer->aimData.vecAimPos[2]);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();

		sprintf(buf, "    Поворот по высоте прицела игрока: %0.2f", g_Players->pLocalPlayer->aimData.fAimZ);
		pD3DFontFixed->PrintShadow(20.0f, y, color, buf);
		(y) += 1.0f + pD3DFontFixed->DrawHeight();
...
void renderSAMP(void)
{
	...
	if (!g_renderSAMP_initSAMPstructs)
	{
		...
		//Log("GIUSTO");
		//Log("Unk0: %X Setg: %X Rak : %X Pools : %X", &g_SAMP->pUnk0, &g_SAMP->pSettings, &g_SAMP->pRakClientInterface, &g_SAMP->pPools);
...
float getFPS(void)
{
	...
	// get fps
	if ((time_get() - MSEC_TO_TIME(240)) > fps_time)
	{
		// for averaging
		fpsBuf4 = fpsBuf3;
		fpsBuf3 = fpsBuf2;
...
void proxyID3DDevice9_InitWindowMode(D3DPRESENT_PARAMETERS *pPresentationParameters)
{
		...
		&& pPresentationParameters->BackBufferHeight != 0)
	{
		...
		if (set.window_mode_titlebar)
		{
			...
			if (um.right == (LONG)pPresentationParameters->BackBufferWidth
				&&	 um.bottom == (LONG)pPresentationParameters->BackBufferHeight) goto proxyID3DDevice9_InitWindowMode_end;
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	float		x = 0.0f;
;
	const uint32_t	color_text = OL_COLOR2(255);//D3DCOLOR_ARGB( 255, 0, 153, 153 );
	const uint32_t	color_enabled = OL_COLOR2(255);//D3DCOLOR_ARGB(255, 0, iColor, iColor);//D3DCOLOR_ARGB( 255, 0, 0, 153 );
	...
	if (isBeginRenderWIN)
	{
		...
		{
			char* CheatName = "";
			float ParWidth = 0, ParHeight = pD3DFont_Footer->DrawHeight(), ParPosY = 0;
...
//					float fWidth = 250.0f;
					...
					pD3DFont_Menu->Print("ПРЕДУПРЕЖДЕНИЕ: Вы используете БЕТА версию собейта. Перевел: Kiko. Тема собейта: Kiko", OL_COLOR5(255),
			...
			else
			{
				...
				static DWORD dwFullProcessTick = time_get();

				if (time_get() - dwFullProcessTick < MSEC_TO_TIME(500))
				{
					render->D3DBox(0, 0, pPresentParam.BackBufferWidth, pPresentParam.BackBufferHeight, 0xFF000000);
					D3DXMATRIX mat;
					D3DXVECTOR2 vecOL_Logo = { ((float)pPresentParam.BackBufferWidth - 250.0f) / 2.0f,
...
//					float fWidth = 250.0f;
				...
				ParPosY = (float)(pPresentParam.BackBufferHeight) - ParHeight - 3.0f;

				if (cheat_state->_generic.hp_cheat)
				{
					CheatName = " Бессмертие ";
				...
				if (set.hud_fps_draw)
				{
					...
					else
					{
						...
						//_snprintf_s(buf, sizeof(buf) - 1, " %0.0f - %0.0f", m_FPS, (float)g_timeDiff);
					}
					//CheatName = "Weapon";
					float fpsPos[2] = { pPresentParam.BackBufferWidth - pD3DFont_Footer->DrawLength(buf),
						pPresentParam.BackBufferHeight - pD3DFont_Footer->DrawHeight() - 3.0f };
					...
					render->D3D_OL_Label(buf, fpsPos[0], fpsPos[1], OL_COLOR4(128), OL_COLOR7(255), OL_COLOR2(255));
			...
			if (cheat_state->state != CHEAT_STATE_NONE)
			{
				if (set.hud_indicator_pos)
				{
					...
					_snprintf_s(buf, sizeof(buf) - 1, " %.2f, %.2f, %.2f  %d ", coord[0], coord[1], coord[2],
						gta_interior_id_get());
					ParWidth = pD3DFont_Footer->DrawLength(buf);
					ParHeight = pD3DFont_Footer->DrawHeight();
					//ParPosY = (float)(pPresentParam.BackBufferHeight) - ParHeight -3.0f;
					render->D3D_OL_Label(buf, x + ParHeight, ParPosY, OL_COLOR4(128), OL_COLOR7(255), OL_COLOR2(255));
					x += ParHeight;
...
{
	if (OLCheats->bChangeGraphics)
	{
		D3DMATERIAL9 * d3dMaterial;

		memcpy(d3dMaterial, pMaterial, sizeof(D3DMATERIAL9));

		d3dMaterial->Ambient.a *= OLCheats->fMatAmbColor;
		d3dMaterial->Ambient.r *= OLCheats->fMatAmbColor;
		d3dMaterial->Ambient.g *= OLCheats->fMatAmbColor;
		d3dMaterial->Ambient.b *= OLCheats->fMatAmbColor;
		//
		d3dMaterial->Diffuse.a *= OLCheats->fMatDiffColor;
		d3dMaterial->Diffuse.g *= OLCheats->fMatDiffColor;
		d3dMaterial->Diffuse.b *= OLCheats->fMatDiffColor;
		d3dMaterial->Diffuse.r *= OLCheats->fMatDiffColor;
		//
		d3dMaterial->Emissive.a *= OLCheats->fMatEmesColor;
		d3dMaterial->Emissive.g *= OLCheats->fMatEmesColor;
		d3dMaterial->Emissive.b *= OLCheats->fMatEmesColor;
		d3dMaterial->Emissive.r *= OLCheats->fMatEmesColor;
		//
		d3dMaterial->Power = OLCheats->fSharpnessPower;
		d3dMaterial->Specular.r = OLCheats->fMatShininess;
		d3dMaterial->Specular.g = OLCheats->fMatShininess;
		d3dMaterial->Specular.b = OLCheats->fMatShininess;
		d3dMaterial->Specular.a = OLCheats->fMatShininess;



		return origIDirect3DDevice9->SetMaterial(d3dMaterial);
	}else
	{
...
HRESULT proxyIDirect3DDevice9::SetLight(DWORD Index, CONST D3DLIGHT9 *pLight)
{
	if (OLCheats->bChangeGraphics)
	{
		...
		memcpy(d3dLight, pLight, sizeof(D3DLIGHT9));
		d3dLight->Specular.r = OLCheats->fLightShininess;
		d3dLight->Specular.g = OLCheats->fLightShininess;
		d3dLight->Specular.b = OLCheats->fLightShininess;
		d3dLight->Specular.a = OLCheats->fLightShininess;

		d3dLight->Range = OLCheats->fCutoffRange;
		d3dLight->Falloff = 0;
		d3dLight->Attenuation0 = 1;
		d3dLight->Attenuation1 = 0;
		d3dLight->Attenuation2 = 0;

		d3dLight->Ambient.a *= OLCheats->fMatAmbColor;
		d3dLight->Ambient.r *= OLCheats->fMatAmbColor;
		d3dLight->Ambient.g *= OLCheats->fMatAmbColor;
		d3dLight->Ambient.b *= OLCheats->fMatAmbColor;
		//
		d3dLight->Diffuse.a *= OLCheats->fMatDiffColor;
		d3dLight->Diffuse.g *= OLCheats->fMatDiffColor;
		d3dLight->Diffuse.b *= OLCheats->fMatDiffColor;
		d3dLight->Diffuse.r *= OLCheats->fMatDiffColor;

		return origIDirect3DDevice9->SetLight(Index, d3dLight);
	}
	else
	{
