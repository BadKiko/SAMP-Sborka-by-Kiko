...
CBotFuncs::CBotFuncs()
{
	...
	BotClient = new stBot[256]();
	memset(&BotSettings, 0, sizeof(stBotSettings));
	uiIsBot = new uint16_t *[SAMP_MAX_PLAYERS];
	for (int i = 0; i < SAMP_MAX_PLAYERS; i++)
		uiIsBot[i] = new uint16_t[2]();
...
void CBotFuncs::OL_Delete_Bot()
{
	...
	if (BotSettings.ClientCount <= 0 || BotSettings.ClientJoined == 0)
	{
		...
		BotSettings.CurrentClientID = 0;
		cheat_state_text("Все боты удалены.");
		return;
	}
	...
	BotClient[BotSettings.ClientCount - 1].
		pRakClient = nullptr;
...
bool CBotFuncs::OL_Add_Bot_Nick(char *nick)
{
	...
	{
		cheat_state_text("Проблема с RakNet ptr");
		Log("Problem With RakNet ptr");
		return false;
	}
	if (!BotClient[BotSettings.ClientCount].pRakClient->Connect(g_SAMP->szIP,
		g_SAMP->ulPort,
		0, 0, BotSettings.UpdateSendRate))
	{
		cheat_state_text("Бот не может присоедениться");
		Log("Bot Client can't connect");
		return false;
	}
	cheat_state_text("Отпраляем запрос соеденения %d",
		BotSettings.ClientCount);
	g_BotFuncs->BotClient[BotSettings.ClientCount].fHealth = 100.0f;
...
void CBotFuncs::OL_Bot_Process()
{
	...
	static DWORD dwUpdateTick = time_get();

	if (time_get() - dwUpdateTick >= MSEC_TO_TIME(BotSettings.UpdateSendRate))
	{
			...
			packet = BotClient[ClientID].pRakClient->Receive())
		{
			...
			case ID_DISCONNECTION_NOTIFICATION:
				cheat_state_text("Сервер закрыл соеденениие. ID Клиента: %d.",
					ClientID);
			...
			case ID_CONNECTION_BANNED:
				cheat_state_text("Вы забанены на этом сервере. ID Клиента: %d.",
					ClientID);
			...
			case ID_CONNECTION_ATTEMPT_FAILED:
				cheat_state_text("Неудачная попытка подключения. ID Клиента: %d.",
					ClientID);
			...
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				cheat_state_text("Сервер полный. ID Клиента: %d.", ClientID);
			...
			case ID_INVALID_PASSWORD:
				cheat_state_text("Неправильный пароль. ID Клиента: %d.",
					ClientID);
			...
			case ID_CONNECTION_LOST:
				cheat_state_text("Потеря соеденения с сервером. ID Клиента: %d.",
					ClientID);
			...
			case ID_MODIFIED_PACKET: // i receive this before ban, maybe is useless
				...
				for (int i = g_BotFuncs->BotSettings.ClientCount; i >= 0; i--)
					OL_Delete_Bot();
				cheat_state_text
				("Боты отключены по причине: Превышено время ожидания/Бан");
		...
		if (BotClient[ClientID].bJoined)
		{
			if (BotSettings.bUseNearestTarget)
				BotSettings.sTargetID = NearestPlayer();

			if (gta_interior_id_get() != BotClient[ClientID].iInteriorID)
				OL_Bot_SetInterior();
			...
			if (BotSettings.bShooter)
				OL_Bot_Shooter();

			if (BotSettings.bBotSilent)
				OL_Bot_Silent();
			if (BotSettings.bBotAttacker)
				OL_Bot_Attacker();
			if (BotSettings.bBotElevator
				|| BotSettings.bBotJetpackElevator)
				OL_Bot_Elevator();
			if (BotSettings.bBotSlapper)
				OL_Bot_Slapper();
			if (BotSettings.bBotFuck)
				OL_Bot_Fuck();
			if (BotSettings.bBotPick)
				OL_Bot_Pick();
			if (BotSettings.bBotFollowAnim)
				OL_Bot_Follow_Anim();


			if (BotSettings.bUIF_DM_Farmer)
				OL_UIF_DM_Farmer();
			if (BotSettings.bBotSun)
				OL_Bot_Sun();

			OL_Bot_Die();
			if (BotSettings.bStorm)
				OL_Bot_Storm();
		}
		BotSettings.CurrentClientID++;
		dwUpdateTick = time_get();
	}
exit_loop:;
...
void CBotFuncs::OL_Bot_SetInterior()
{

	BitStream bsInteriorDef;
	bsInteriorDef.Write((BYTE)0);
	OL_Bot_SendRPC(BotSettings.CurrentClientID, RPC_SetInteriorId, bsInteriorDef,
		HIGH_PRIORITY, RELIABLE_SEQUENCED, NULL, false);

	BitStream bsInterior;
	bsInterior.Write((BYTE)gta_interior_id_get());
	OL_Bot_SendRPC(BotSettings.CurrentClientID, RPC_SetInteriorId, bsInterior,
		HIGH_PRIORITY, RELIABLE_SEQUENCED, NULL, false);
	BotClient[BotSettings.CurrentClientID].iInteriorID = gta_interior_id_get();
...
void OL_Bot_SetHealth(RPCParameters * params)
{
	...
	bsData.Read(g_BotFuncs->BotClient[g_BotFuncs->BotSettings.CurrentClientID].
		fHealth);
	cheat_state_text("Здоровье выставлено на: %.2f", g_BotFuncs->BotClient[g_BotFuncs->BotSettings.CurrentClientID].
		fHealth);
}
...
void CBotFuncs::OL_Bot_Send_Aim()
{
	BitStream bsSendCommand;

	if (time_get() -
		BotClient[BotSettings.CurrentClientID].dwSendAimTick > MSEC_TO_TIME(1000))
	{
		stAimData AimData;
		memset(&AimData, 0, sizeof(stAimData));
		vect3_copy(g_BotFuncs->BotClient[BotSettings.CurrentClientID].
			fLastOnFootPos, AimData.vecAimPos);
		AimData.vecAimf1[0] = 0.0f;//(float)((rand() % 200) / 100.0f) - 1.0f;
		AimData.vecAimf1[1] = 0.0f;//(float)((rand() % 200) / 100.0f) - 1.0f;
		AimData.vecAimf1[2] = 1.0f;//(float)((rand() % 200) / 100.0f) - 1.0f;
		BitStream bsSend;
		bsSend.Write((BYTE)ID_AIM_SYNC);
		bsSend.Write((PCHAR)& AimData, sizeof(stAimData));
		g_BotFuncs->BotClient[BotSettings.CurrentClientID].pRakClient->Send(&bsSend,
			HIGH_PRIORITY,
			UNRELIABLE_SEQUENCED,
			0);

		BotClient[BotSettings.CurrentClientID].dwSendAimTick =
			time_get();
	}
}
...
void CBotFuncs::OL_UIF_DM_Farmer()
{
	...
	if (time_get() -
		BotClient[BotSettings.CurrentClientID].dwSendBulletTick > MSEC_TO_TIME(2000))
	{
...
{
	//      float fQuaternion[4];
	if (IsPlayerStreamed(BotSettings.sTargetID))
	{
		const uint16_t playerID = BotSettings.sTargetID;
		stOnFootData OnFoot;
		memset(&OnFoot, 0, sizeof(stOnFootData));
		OnFoot.byteHealth =
			g_BotFuncs->BotClient[BotSettings.CurrentClientID].fHealth;
...
void CBotFuncs::OL_Packet_PlayerSync(Packet * packet, uint8_t ClientID)
{
	if (g_BotFuncs->BotSettings.bBotSilent)
	{
		...
		if (bHasSurfInfo)
		{
			...
			OnFootData.sSurfingVehicleID = (uint16_t)-1;
...
void CBotFuncs::OL_Bot_Follow_OnFoot(bool bFake)
{
	...
	float rest = (BotSettings.CurrentClientID + 1) & 1;
...
void CBotFuncs::OL_Bot_Follow()
{
	...
	if (BotSettings.UseFakeBot)
	{
		...
		{
			cheat_state_text("Игрок вышел из стрима");
			return;
		}
	}
	else
...
{
	Log("Авторизация ключа для ID Клиента: %d", ClientID);
	char *auth_key = "";
...
{
	cheat_state_text("Попытка соеденения для ID Клиента: %d", ClientID);
...
void CBotFuncs::OL_Random_Spawn_Bot(uint8_t ClientID)
{
	...
	OL_Bot_SendRPC(ClientID, RPC_Spawn, bsSendSpawn, HIGH_PRIORITY, RELIABLE,
		0, FALSE);
	g_BotFuncs->BotClient[BotSettings.CurrentClientID].fHealth = 100.0f;
	BotClient[BotSettings.CurrentClientID].iInteriorID = 0;
}
