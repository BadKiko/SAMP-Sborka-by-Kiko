...
int OverLight_Bot_Stuff_Callback(int op, struct menu_item *item)
{
	...
	case MENU_OP_ENABLED:
		...
		case ID_INCREASE_PING:
			snprintf(name, sizeof(name), "\aПинг: %d", g_BotFuncs->BotSettings.incrasePing);
			menu_item_name_set(item, name);
			break;
...
int OverLight_Change_Graphics_Callback(int op, struct menu_item *item)
{
	...
	case MENU_OP_ENABLED:
		switch (item->id)
		{
		case ID_CHANGE_GRAPHICS:
			return OLCheats->bChangeGraphics;
	...
	case MENU_OP_INC:
		...
		case ID_SHARPNESS_POWER:
		{
			OLCheats->fSharpnessPower += increase * 0.25f;
			break;
		}

		case ID_LIGHT_RANGE:
		{
			OLCheats->fCutoffRange += increase * 50;
			break;
		}
		case ID_MAT_SHININESS:
		{
			OLCheats->fMatShininess += increase * 0.25f;
			break;
		}
		case ID_LIGHT_SHININESS:
		{
			OLCheats->fLightShininess += increase * 0.25f;
			break;
		}

		case ID_MAT_AMBIENTCOLOR:
		{
			OLCheats->fMatAmbColor += increase * 0.25f;
			break;
		}

		case ID_MAT_DIFFUSECOLOR:
		{
			OLCheats->fMatDiffColor += increase * 0.25f;
			break;
		}
		case ID_MAT_EMESSIVECOLOR:
		{
			OLCheats->fMatEmesColor += increase * 0.25f;
			break;
		}

		case ID_LIGHT_AMBIENTCOLOR:
		{
			OLCheats->fLightAmbColor += increase * 0.25f;
			break;
		}

		case ID_LIGHT_DIFFUSECOLOR:
		{
			OLCheats->fLightDiffColor += increase * 0.25f;
			break;
		}
