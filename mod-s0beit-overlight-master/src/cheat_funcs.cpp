...
const struct weapon_entry	weapon_list[] =
{
	{ 0, 0, -1, "Кулаки" },
	{ 1, 0, 331, "Кастет" },
	{ 2, 1, 333, "Клюшка для гольфа" },
	{ 3, 1, 334, "Полицейская дубинка" },
	{ 4, 1, 335, "Нож" },
	{ 5, 1, 336, "Бейсбольная бита" },
	{ 6, 1, 337, "Лопата" },
	{ 7, 1, 338, "Бильярдный кий" },
	{ 8, 1, 339, "Катана" },
	{ 9, 1, 341, "Бензопила" },
	{ 22, 2, 346, "Пистолет" },
	{ 23, 2, 347, "Пистолет с глушителем" },
	{ 24, 2, 348, "Дигл" },
	{ 25, 3, 349, "Дробовик" },
	{ 26, 3, 350, "Обрез" },
	{ 27, 3, 351, "SPAZ12" },
	...
	{ 31, 5, 356, "M4" },
	{ 33, 6, 357, "Ружьё" },
	{ 34, 6, 358, "Снайперская винтовка" },
	{ 35, 7, 359, "РПГ-7" },
	{ 36, 7, 360, "Стингер" },
	{ 37, 7, 361, "Огнемёт" },
	{ 38, 7, 362, "Миниган" },
	{ 16, 8, 342, "Осколочная граната" },
	{ 17, 8, 343, "Слезоточивый газ" },
	{ 18, 8, 344, "Коктейль Молотова" },
	{ 39, 8, 363, "Взрывчатка на ДУ" },
	{ 41, 9, 365, "Аэрозольный баллончик" },
	{ 42, 9, 366, "Огнетушитель" },
	{ 43, 9, 367, "Камера" },
	{ 10, 10, 321, "Фаллоимитатор 1" },
	{ 11, 10, 322, "Фаллоимитатор 2" },
	{ 12, 10, 323, "Вибратор  1" },
	{ 13, 10, 324, "Вибратор  2" },
	{ 14, 10, 325, "Цветы" },
	{ 15, 10, 326, "Трость" },
	{ 44, 11, 368, "Прибор ночного видения" },
	{ 45, 11, 369, "Тепловизор" },
	{ 46, 11, 371, "Парашют" },
	{ 40, 12, 364, "Пульт ДУ взрывчаткой" },
	{ -1, -1, -1, NULL }
};
...
{
	{ 11, 2003.1178f, 1015.1948f, 33.008f, 351.5789f, "Четыре дракона" },
	{ 5, 770.8033f, -0.7033f, 1000.7267f, 22.8599f, "Тренажёрный зал ЛС" },
	...
	{ 7, 773.7318f, -74.6957f, 1000.6542f, 5.2304f, "Below The Belt Gym (LV)" },
	...
	{ 14, -1864.9434f, 55.7325f, 1055.5276f, 85.8541f, "ЛС Аирпорт" },
...
int memcpy_safe(void *_dest, const void *_src, uint32_t len, int check, const void *checkdata)
{
	...
	const uint8_t	*src = (const uint8_t *)_src;
	int				ret = 1;
	traceLastFunc("memcpy_safe 2");
	if (check && checkdata)
	{
		traceLastFunc("memcpy_safe 3");
	...
	while (len > 0)
	{
		traceLastFunc("memcpy_safe 4");
		uint32_t	this_len = sizeof(buf);

		if (this_len > len)
			this_len = len;

		traceLastFunc("memcpy_safe 5");
		if (!__page_read(buf, src, this_len))
			ret = 0;

		traceLastFunc("memcpy_safe 6");
...
{
	traceLastFunc("memcmp_safe 1");
	const uint8_t	*s1 = (const uint8_t *)_s1;
	traceLastFunc("memcmp_safe 2");
	const uint8_t	*s2 = (const uint8_t *)_s2;
	traceLastFunc("memcmp_safe 3");
	uint8_t			buf[4096];

	traceLastFunc("memcmp_safe 4");

	for (;; )
	{
		traceLastFunc("memcmp_safe 5");
		if (len > 4096)
		{
			traceLastFunc("memcmp_safe 6 1 1");
			if (!memcpy_safe(buf, s1, 4096))
				return 0;
			traceLastFunc("memcmp_safe 6 1 2");
			if (memcmp(buf, s2, 4096))
				return 0;
			traceLastFunc("memcmp_safe 6 1 3");
			s1 += 4096;
			s2 += 4096;
			len -= 4096;
		}
		...
		{
			traceLastFunc("memcmp_safe 6 2 1");
			if (!memcpy_safe(buf, s1, len))
				return 0;
			traceLastFunc("memcmp_safe 6 2 2");
